...
const SplineAbout = ({
  ...
  useGSAP(
    () => {
      ...
      if (badgeWrapperRef.current) tl.to(badgeWrapperRef.current, { autoAlpha: 1, y: 0, duration: 0.8 }, 0);

      if (headingRef.current) gsap.set(headingRef.current, { autoAlpha: 0, y: 30, scale: 0.95 });
      if (headingRef.current) tl.to(headingRef.current, { autoAlpha: 1, y: 0, scale: 1, duration: 1 }, 0.2);

      if (paragraphRef.current) gsap.set(paragraphRef.current, { autoAlpha: 0, y: 20 });
      if (paragraphRef.current) tl.to(paragraphRef.current, { autoAlpha: 1, y: 0, duration: 0.8 }, 0.4);
      ...
      if (cardsHeaderRef.current) gsap.set(cardsHeaderRef.current, { autoAlpha: 0, y: 20 });
      if (cardsHeaderRef.current) tl.to(cardsHeaderRef.current, { autoAlpha: 1, y: 0, duration: 0.8 }, 0.8);
  ...
  return (
    ...
    >
      {/* Background with Emerald Color (0.2, 0.7, 0.5) ~ #33b380 */}
      <ShaderBackground color="#33b380" alpha={1.0} />
